...
const DarkDashboard: React.FC<DashboardProps> = ({ theme }) => {
  const { connectWallet, disconnectWallet, isConnected, account, balance, chainId, networkConfig, web3, supplyChainContract } = useWeb3();
  ...
  const [recentActivity, setRecentActivity] = useState([
    { type: 'success', message: 'Product batch verified successfully', time: '2 min ago' },
    { type: 'info', message: 'New shipment started tracking', time: '15 min ago' },
    { type: 'warning', message: 'Temperature alert triggered', time: '1 hour ago' },
    { type: 'info', message: 'IoT sensor connected', time: '2 hours ago' }
  ]);

  // Fetch blockchain data when connected
  useEffect(() => {
    if (isConnected && supplyChainContract) {
      fetchBlockchainData();
    }
  }, [isConnected, supplyChainContract]);
  ...
  const fetchBlockchainData = async () => {
    ...
    try {
      ...
      if (web3 && supplyChainContract) {
        ...
        const timestamp = await web3.eth.getBlock('latest').then(block => block.timestamp);
        ...
        if (isConnected) {
          setRecentActivity(prev => [
            { 
              type: 'success', 
              message: `Wallet ${account?.substring(0, 6)}...${account?.substring(account.length - 4)} connected`, 
              time: 'just now' 
            },
            ...prev.slice(0, 3)
          ]);
  ...
  const handleTrackShipment = () => {
    ...
    setRecentActivity(prev => [
      { type: 'info', message: 'Opening shipment tracker...', time: 'just now' },
      ...prev.slice(0, 3)
    ]);
  ...
  return (
    <div className="p-6 min-h-screen bg-gray-900 text-white">
        ...
        <div className="flex items-center space-x-4">
          ...
          {!isConnected ? (
            <button
              ...
              className={`${isConnecting ? 'bg-blue-800' : 'bg-blue-600 hover:bg-blue-700'} text-white px-4 py-2 rounded-lg font-medium flex items-center`}
            >
              {isConnecting ? (
                <>
            ...
            <div className="flex items-center space-x-2">
              <div className="bg-gray-800 px-3 py-1 rounded-lg text-sm flex items-center">
                ...
                {chainId && <span className="ml-2 text-xs bg-blue-900 px-2 py-0.5 rounded">{networkConfig?.name || `Chain: ${chainId}`}</span>}
              </div>
              <button 
      ...
      <div className="flex items-center space-x-6 mb-8 border-b border-gray-800 pb-2">
        <a href="#/" className="text-blue-500 border-b-2 border-blue-500 pb-2 font-medium">
          <span className="mr-1">🏠</span> Overview
        </a>
        <a href="#/products" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          window.location.href = "#/products";
          setRecentActivity(prev => [{ type: 'info', message: 'Viewing products list', time: 'just now' }, ...prev.slice(0, 3)]);
        }}>
          <span className="mr-1">📦</span> Products
        </a>
        <a href="#/create-product" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          handleCreateProduct();
        }}>
          <span className="mr-1">✅</span> Create Product
        </a>
        <a href="#/scan" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          handleScanVerify();
        }}>
          <span className="mr-1">🔍</span> Scan & Verify
        </a>
        <a href="#/track" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          handleTrackShipment();
        }}>
          <span className="mr-1">🌎</span> GPS Tracking
        </a>
        <a href="#/iot" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          handleIoTMonitor();
        }}>
          <span className="mr-1">🌡️</span> IoT Dashboard
        </a>
        <a href="#/analytics" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          window.location.href = "#/analytics";
          setRecentActivity(prev => [{ type: 'info', message: 'Viewing analytics dashboard', time: 'just now' }, ...prev.slice(0, 3)]);
        }}>
          <span className="mr-1">📊</span> Analytics
        </a>
        <a href="#/recalls" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          window.location.href = "#/recalls";
          setRecentActivity(prev => [{ type: 'info', message: 'Checking product recalls', time: 'just now' }, ...prev.slice(0, 3)]);
        }}>
          <span className="mr-1">⚠️</span> Recalls
        </a>
        <a href="#/notifications" className="text-gray-400 hover:text-white pb-2" onClick={(e) => {
          e.preventDefault();
          window.location.href = "#/notifications";
          setRecentActivity(prev => [{ type: 'info', message: 'Viewing notifications', time: 'just now' }, ...prev.slice(0, 3)]);
        }}>
      ...
      <div className="grid grid-cols-4 gap-6 mb-8">
        <div className={`bg-gray-800 rounded-xl p-6 border border-gray-700 ${isLoading ? 'animate-pulse' : ''}`}>
          <div className="flex justify-between items-start">
            <div>
              <p className="text-gray-400 mb-1">Total Products</p>
              <h3 className="text-3xl font-bold text-white">{totalProducts.toLocaleString()}</h3>
        ...
        <div className={`bg-gray-800 rounded-xl p-6 border border-gray-700 ${isLoading ? 'animate-pulse' : ''}`}>
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-white">{verifiedItems.toLocaleString()}</h3>
              <p className="text-green-500 text-sm">{Math.round((verifiedItems/totalProducts)*100)}%</p>
            </div>
        ...
        <div className={`bg-gray-800 rounded-xl p-6 border border-gray-700 ${isLoading ? 'animate-pulse' : ''}`}>
          <div className="flex justify-between items-start">
            <div>
              ...
              <h3 className="text-3xl font-bold text-white">{activeShipments}</h3>
        ...
        <div className={`bg-gray-800 rounded-xl p-6 border border-gray-700 ${isLoading ? 'animate-pulse' : ''}`}>
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-red-500 text-sm">{alerts > 0 ? `${alerts} Alerts` : 'Normal'}</p>
            </div>
            ...
            {isConnected && (
              <button 
                onClick={() => {
                  ...
                  const activities = [
                    { type: 'info', message: 'New temperature reading received', time: 'just now' },
                    { type: 'success', message: 'Blockchain transaction confirmed', time: 'just now' },
                    { type: 'warning', message: 'Shipment delay detected', time: 'just now' }
                  ];
