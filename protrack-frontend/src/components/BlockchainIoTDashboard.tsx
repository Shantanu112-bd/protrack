...
const BlockchainIoTDashboard: React.FC<BlockchainIoTDashboardProps> = ({ isDark }) => {
  const { submitIoTData, submitGPSData, verifyIoTData, monitorSLA } = useBlockchain()
  ...
  useEffect(() => {
    ...
    const generateDemoData = () => {
      ...
      const sensorTypes = ['temperature', 'humidity', 'pressure', 'vibration'] as const
  ...
  const handleSubmitIoTData = async (deviceId: string, sensorType: string, value: number) => {
  ...
  const handleMonitorSLA = async () => {
    ...
    try {
      const result = await monitorSLA('SHIP-001', {
        ...
        requiredLocation: { lat: 40.7128, lon: -74.0060, radius: 10 }
      })
      setSlaStatus(result)
    } catch (error) {
  ...
  const getValueColor = (sensorType: string, value: number, verified: boolean) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className={`${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
            Real-time sensor data with blockchain verification and oracle integration
          </p>
        </div>
        <button
      ...
      {!isConnected && (
        <div className="mb-6 p-4 bg-yellow-500/20 border border-yellow-500/30 rounded-lg">
          <p className={`${isDark ? 'text-yellow-200' : 'text-yellow-800'} text-center`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {iotData.map((sensor, index) => (
          <div key={sensor.deviceId} className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-6 shadow-sm border`}>
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full ${sensor.verified ? 'bg-green-500' : 'bg-red-500'}`}></div>
                <span className={`text-xs ${sensor.verified ? 'text-green-600' : 'text-red-600'}`}>
            ...
            <div className="mb-4">
              <span className={`text-3xl font-bold ${getValueColor(sensor.sensorType, sensor.value, sensor.verified)}`}>
                {sensor.value}
              </span>
              <span className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-500'} ml-1`}>
                {sensor.unit}
              </span>
            </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              {sensor.verified && sensor.txHash && (
                <div className="flex justify-between">
                  <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>Tx Hash:</span>
              ...
              {sensor.verified && sensor.blockNumber && (
                <div className="flex justify-between">
                  <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>Block:</span>
                  <span className={isDark ? 'text-white' : 'text-gray-900'}>
            ...
            <button
              onClick={() => handleSubmitIoTData(sensor.deviceId, sensor.sensorType, sensor.value)}
      ...
      <div className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-6 shadow-sm border mb-8`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {gpsData.map((gps, index) => (
            <div key={gps.shipmentId} className={`${isDark ? 'bg-gray-700' : 'bg-gray-50'} rounded-lg p-4`}>
              <div className="flex items-center justify-between mb-3">
                <h4 className={`font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  {gps.shipmentId}
                </h4>
                <div className={`w-3 h-3 rounded-full ${gps.verified ? 'bg-green-500' : 'bg-red-500'}`}></div>
              </div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`${gps.verified ? 'text-green-600' : 'text-red-600'}`}>
                    {gps.verified ? 'Blockchain Verified' : 'Pending Verification'}
                  </span>
                </div>
                {gps.verified && gps.txHash && (
                  <div className="flex justify-between">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>Tx:</span>
        ...
        <div className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-6 shadow-sm border`}>
          ...
          {slaStatus.violations.length > 0 && (
            <div className="space-y-2 mb-4">
              <h4 className={`font-medium ${isDark ? 'text-red-400' : 'text-red-600'}`}>
                Violations:
              </h4>
              {slaStatus.violations.map((violation: string, index: number) => (
                <div key={index} className={`flex items-center space-x-2 ${isDark ? 'text-red-300' : 'text-red-700'}`}>
                  <span>⚠️</span>
          ...
          {slaStatus.penaltyAmount && (
            <div className={`${isDark ? 'bg-red-900/20 border-red-700' : 'bg-red-50 border-red-200'} border rounded-lg p-4`}>
              <div className="flex items-center justify-between">
                ...
                <span className={`text-lg font-bold ${isDark ? 'text-red-400' : 'text-red-800'}`}>
