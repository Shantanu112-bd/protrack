        ...
        <TabsContent value="inventory">
          <Card>
            ...
            <CardContent>
              ...
              ) : inventory.filter(item => item.status === 'at_retailer').length === 0 ? (
                <p>No products in inventory.</p>
              ) : (
                <div className="grid gap-4">
                  {inventory.filter(item => item.status === 'at_retailer').map((product) => (
                    <Card key={product.id} className="p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-medium">{product.product_name || 'Product'}</h3>
            ...
            <CardContent>
              ...
              {scannedProduct && (
                <Card className="mt-4 p-4">
                  <CardTitle className="text-lg mb-2">Product Information</CardTitle>
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2">
                      ...
                      <span className="text-sm">{scannedProduct.manufacturer?.name || 'Unknown'}</span>
                    </div>
                    <div className="grid grid-cols-2">
                      <span className="text-sm font-medium">Expiry Date:</span>
                      <span className="text-sm">
                        {new Date(scannedProduct.expiry_date).toLocaleDateString()}
                      </span>
                    </div>
                    <div className="grid grid-cols-2">
        ...
        <TabsContent value="incoming">
          <Card>
            ...
            <CardContent>
              ...
              ) : inventory.filter(item => item.status === 'in_transit').length === 0 ? (
                <p>No incoming shipments.</p>
              ) : (
                <div className="grid gap-4">
                  {inventory.filter(item => item.status === 'in_transit').map((product) => (
                    <Card key={product.id} className="p-4">
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-medium">{product.product_name || 'Product'}</h3>
                          <p className="text-sm text-gray-500">RFID: {product.rfid_tag}</p>
                          <p className="text-sm text-gray-500">From: {product.manufacturer?.name || 'Unknown'}</p>
                          <p className="text-sm text-gray-500">
                            Expected: {product.expected_arrival ? new Date(product.expected_arrival).toLocaleDateString() : 'Unknown'}
                          </p>
                          <div className="mt-2">
