...
const BlockchainProductScanner: React.FC<BlockchainProductScannerProps> = ({ isDark }) => {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      const verificationScore = calculateVerificationScore(productData, supplyChainHistory)
  ...
  const calculateVerificationScore = (productData: any, history: any[]): number => {
    let score = 100
    ...
    const daysSinceCreation = (Date.now() / 1000 - productData.createdAt) / (24 * 60 * 60)
  ...
  const identifyRiskFactors = (productData: any, history: any[]): string[] => {
    ...
    const tempViolations = history.filter(event => 
  ...
  const simulateQRScan = () => {
    ...
    const randomTokenId = demoTokenIds[Math.floor(Math.random() * demoTokenIds.length)]
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Scanning Interface */}
      <div className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-8 shadow-sm border mb-8`}>
        ...
        <div className="max-w-md mx-auto">
          {scanMethod === 'qr' && (
            <div className="text-center">
              <div className={`${isDark ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-300'} border-2 border-dashed rounded-xl p-12 mb-6`}>
        ...
        <div className="space-y-6">
          ...
          <div className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl p-6 shadow-sm border`}>
            <div className="flex items-center justify-between mb-6">
              ...
              <div className={`px-4 py-2 rounded-full border-2 ${getScoreBgColor(verificationResult.verificationScore)}`}>
                <span className={`text-2xl font-bold ${getScoreColor(verificationResult.verificationScore)}`}>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>Token ID:</span>
                  ...
                  <div className="flex justify-between">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>SKU:</span>
                    <span className={`font-mono ${isDark ? 'text-white' : 'text-gray-900'}`}>
                      {verificationResult.productData.sku}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className={isDark ? 'text-gray-400' : 'text-gray-500'}>Batch ID:</span>
                    <span className={`font-mono ${isDark ? 'text-white' : 'text-gray-900'}`}>
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className={isDark ? 'text-white' : 'text-gray-900'}>
                      {new Date(verificationResult.productData.createdAt * 1000).toLocaleDateString()}
                    </span>
                  </div>
                </div>
              </div>
            ...
            {verificationResult.riskFactors.length > 0 && (
              <div className="mt-6 p-4 bg-yellow-500/20 border border-yellow-500/30 rounded-lg">
                <h4 className={`font-semibold ${isDark ? 'text-yellow-200' : 'text-yellow-800'} mb-2`}>
                  ⚠️ Risk Factors Identified
                </h4>
                <ul className="space-y-1">
                  {verificationResult.riskFactors.map((risk, index) => (
                    <li key={index} className={`text-sm ${isDark ? 'text-yellow-200' : 'text-yellow-700'}`}>
                      • {risk}
                    </li>
                  ))}
                </ul>
